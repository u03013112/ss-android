...
There are no arbitrary restrictions/requirements on package name, component name and content
 ...
 name, use `ss.plugin.$PLUGIN_ID` if it only contains a single plugin to prevent
...
* MUST have action: `ss.plugin.ACTION_CONFIGURE`;
* MUST have category: `android.intent.category.DEFAULT`;
* MUST be able to receive data URI `plugin://ss/$PLUGIN_ID`;
* SHOULD parse string extra `ss.plugin.EXTRA_OPTIONS` (all options as a single
...
This corresponds to `ss.plugin.ConfigurationActivity` in the plugin library.
...
* MUST have action: `ss.plugin.ACTION_HELP`;
* MUST have category: `android.intent.category.DEFAULT`;
* MUST be able to receive data URI `plugin://ss/$PLUGIN_ID`;
* CAN parse string extra `ss.plugin.EXTRA_OPTIONS` and display some more
  relevant information;
* SHOULD parse `@NightMode` int extra `ss.plugin.EXTRA_NIGHT_MODE` and act
  accordingly;
* SHOULD either:
  - Be invisible and return help message with CharSequence extra
    `ss.plugin.EXTRA_HELP_MESSAGE` in the data intent with `RESULT_OK`; (in this
...
This corresponds to `ss.plugin.HelpActivity` or
 `ss.plugin.HelpCallback` in the plugin library. Here's what a proper help
...
* MUST have an intent filter with action `ss.plugin.ACTION_NATIVE_PLUGIN`;
  (used for discovering plugins)
* MUST have meta-data `ss.plugin.id` with string value `$PLUGIN_ID`;
* MUST have an intent filter with action `ss.plugin.ACTION_NATIVE_PLUGIN` and
  data `plugin://ss/$PLUGIN_ID`; (used for configuring plugin)
* CAN have meta-data `ss.plugin.default_config` with string value, default is
  empty;
...
* MUST implement `openFile` that for files returned in `query`, `openFile` with `mode = "r"` returns
  ...
  `content://ss.plugin.kcptun/kcptun`.

This corresponds to `ss.plugin.NativePluginProvider` in the plugin library.
...
* MUST implement `call` that returns absolute path to the entry executable as
  `ss.plugin.EXTRA_ENTRY` when having `method = "shadowsocks:getExecutable"`;
  (`ss.plugin.EXTRA_OPTIONS` is provided in extras as well just in case you
  need them)
...
Additionally, if your plugin only needs to supply the path of your executable without doing any extra setup work,
 you can use an additional `meta-data` with name `ss.plugin.executable_path`
...
```
<meta-data android:name="ss.plugin.version"
           android:value="1.0.0"/>
```
